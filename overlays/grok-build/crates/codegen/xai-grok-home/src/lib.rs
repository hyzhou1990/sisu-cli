...
/// Prefer a non-empty `GROK_HOME` over `SISU_HOME` so the engine home wins
/// if both leak onto the process.
fn resolve_grok_home_from_envs(
    grok_home_env: Option<&OsStr>,
    sisu_home_env: Option<&OsStr>,
    os_home: Option<&Path>,
) -> Option<PathBuf> {
    let grok = grok_home_env.filter(|value| !value.is_empty());
    if grok.is_some() {
        return resolve_grok_home_from(grok, os_home);
    }
    let sisu = sisu_home_env.filter(|value| !value.is_empty());
    resolve_grok_home_from(sisu, os_home)
}
...
pub fn resolve_grok_home() -> Option<PathBuf> {
    resolve_grok_home_from_envs(
        std::env::var_os("GROK_HOME").as_deref(),
        std::env::var_os("SISU_HOME").as_deref(),
...
mod tests {
    ...
    fn grok_home_wins_when_both_set() {
        ...
        let resolved =
            resolve_grok_home_from_envs(Some(grok), Some(sisu), Some(Path::new("/home/u")));
        assert_eq!(resolved, Some(PathBuf::from("/tmp/engine")));
    }
}
