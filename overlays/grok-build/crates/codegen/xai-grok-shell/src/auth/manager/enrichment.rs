...
async fn fetch_user_info(manager: &AuthManager, key: &str, log_label: &str) -> Option<UserInfo> {
    ...
    let mut request = http_client.get(&user_url).timeout(USER_FETCH_TIMEOUT);
    match crate::sisu_access_point::access_point_authorization_for_url(&user_url) {
        ...
        None => {
            request = request
                ...
                .header(
                    "X-XAI-Token-Auth",
                    manager.grok_com_config.token_header.as_str(),
                );
        }
    }
