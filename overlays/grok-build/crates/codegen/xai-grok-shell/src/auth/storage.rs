...
/// `GROK_AUTH_PATH` when set, else `$grok_home/auth.json`. Never SiSu identity.
fn api_key_auth_path(grok_home: &Path) -> PathBuf {
...
pub fn read_api_key(grok_home: &Path) -> Option<String> {
    let path = api_key_auth_path(grok_home);
...
mod write_fallback_tests {
    ...
    #[test]
    #[serial_test::serial]
    fn store_api_key_honors_grok_auth_path() {
        use xai_grok_test_support::EnvGuard;
        let dir = tempfile::tempdir().unwrap();
        let grok_home = dir.path().join("engine");
        let sisu_auth = dir.path().join("auth.json");
        let engine_auth = grok_home.join("auth.json");
        std::fs::create_dir_all(&grok_home).unwrap();
        std::fs::write(&sisu_auth, r#"{"token":"sisu-jwt"}"#).unwrap();
        let _path = EnvGuard::set("GROK_AUTH_PATH", engine_auth.as_os_str());
        store_api_key(&grok_home, "engine-key").unwrap();
        assert_eq!(read_api_key(&grok_home).as_deref(), Some("engine-key"));
        assert!(engine_auth.exists());
        let sisu = std::fs::read_to_string(&sisu_auth).unwrap();
        assert!(sisu.contains("sisu-jwt"), "must not write SiSu identity");
        assert!(!sisu.contains("engine-key"));
    }
}
