...
impl HttpAuth for ShellAuthCredentialProvider {
    fn apply(&self, builder: RequestBuilder, base_url: &str) -> RequestBuilder {
        if let Some(override_auth) =
            crate::sisu_access_point::access_point_authorization_for_url(base_url)
        {
            return match override_auth {
                Some(authz) => builder.header("Authorization", authz),
                None => builder,
            };
        }
    ...
    fn snapshot(&self) -> CredentialSnapshot {
        if crate::sisu_access_point::active() {
            return CredentialSnapshot {
                token: crate::sisu_access_point::sisu_token(),
                ..Default::default()
            };
        }
