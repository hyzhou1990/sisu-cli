...
/// Authorization material for a billing upstream call.
///
/// SiSu access-point: the grok AuthStore is intentionally empty (the host
/// owns the session JWT via env), so `require_xai_auth` would always fail and
/// every billing surface — the pager's status bar, `/usage`, and the welcome
/// quota line — would stay empty. Send the host SiSu JWT instead (empty when
/// the host supplied none); sisu quota is enforced server-side per request.
/// Stock mode keeps the grok AuthStore gate and its bearer headers.
enum BillingAuth {
    /// grok AuthStore credential (stock path).
    Grok { key: String, user_id: String },
    /// SiSu access-point: ready-to-send `Authorization` value.
    AccessPoint { authorization: String },
}

fn resolve_billing_auth(
    agent: &MvpAgent,
    missing_message: &'static str,
    non_xai_message: &'static str,
) -> Result<BillingAuth, acp::Error> {
    if crate::sisu_access_point::active() {
        return Ok(BillingAuth::AccessPoint {
            authorization: crate::sisu_access_point::access_point_authorization()
                .unwrap_or_default(),
        });
    }
    let auth =
        super::auth_gate::require_xai_auth(&agent.auth_manager, missing_message, non_xai_message)?;
    Ok(BillingAuth::Grok {
        key: auth.key,
...
/// Stamp the auth-dependent headers: grok bearer + `x-userid` from the
/// AuthStore, or the SiSu session JWT with the grok-only `x-userid` omitted.
fn with_billing_auth(
    builder: reqwest::RequestBuilder,
    auth: &BillingAuth,
) -> reqwest::RequestBuilder {
    match auth {
        BillingAuth::Grok { key, user_id } => builder
        ...
        BillingAuth::AccessPoint { authorization } => {
...
async fn handle_get_billing(agent: &MvpAgent) -> ExtResult {
    // SiSu access-point resolves the host session JWT instead of the empty
    // grok AuthStore (see `resolve_billing_auth`).
    let auth = resolve_billing_auth(
        agent,
    ...
    let credits_resp = with_billing_auth(crate::http::shared_client().get(&credits_url), &auth)
...
async fn handle_get_auto_topup_rule(agent: &MvpAgent) -> ExtResult {
    ...
    let response = with_billing_auth(crate::http::shared_client().get(&url), &auth)
        .header("x-grok-client-version", xai_grok_version::VERSION)
        .header(
            crate::http::CLIENT_MODE_HEADER,
            crate::http::process_client_mode(),
