...
async fn handle_check_subscription(agent: &MvpAgent) -> ExtResult {
    ...
    // owns the session JWT via env — so the stock path below always produced
    // `meta: null` and the pager logged `subscription.check.meta_parse_failed`
    // on every watch poll. Answer from the host-provided env instead. SiSu
    // quota is enforced server-side per request, never through this gate.
    if crate::sisu_access_point::active() {
        let meta = serde_json::json!({
            "email": crate::sisu_access_point::account_email(),
            "auth_mode": "api_key",
            "coding_data_retention_opt_out": true,
            "subscription_tier": crate::sisu_access_point::account_plan(),
        });
        return to_raw_response(&serde_json::json!({
            "authenticated": crate::sisu_access_point::sisu_token().is_some(),
            "meta": meta,
        }));
    }
