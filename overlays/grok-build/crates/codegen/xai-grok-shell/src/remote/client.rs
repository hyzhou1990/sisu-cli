...
pub(crate) fn fetch_models_blocking(
    ...
    match source.auth {
        EndpointAuth::ApiKey => {
            let api_key = if crate::sisu_access_point::active() {
                crate::sisu_access_point::sisu_token().ok_or_else(|| {
                    BackendError::Auth("No SiSu session token. Run `sisu login`.".into())
                })?
            } else {
...
pub(crate) fn parse_remote_model_value(
    ...
    let catalog_base_url = get_string(obj, "baseUrl")
        .or_else(|| get_string(obj, "base_url"))
        .unwrap_or_else(|| default_base_url.to_owned());
    let base_url = crate::sisu_access_point::pin_runtime_url(&catalog_base_url)
        .unwrap_or_else(|| default_base_url.to_owned());
    ...
    Some(crate::agent::config::ModelEntryConfig {
        ...
        api_key: if crate::sisu_access_point::active() {
            None
        } else {
            get_string(obj, "apiKey").or_else(|| get_string(obj, "api_key"))
        },
        env_key: if crate::sisu_access_point::active() {
            None
        } else {
            get_env_keys(obj, "envKey").or_else(|| get_env_keys(obj, "env_key"))
        },
        api_backend,
        context_window,
        ...
        api_base_url: if crate::sisu_access_point::active() {
            crate::sisu_access_point::pin_runtime_url(default_base_url)
        } else {
            get_string(obj, "apiBaseUrl").or_else(|| get_string(obj, "api_base_url"))
        },
        use_concise: obj
            .get("useConcise")
