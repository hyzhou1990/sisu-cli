...
#[test]
fn access_point_parse_ignores_catalog_xai_base_and_keys() {
    ...
    let result = parse_remote_model_value(&value, "https://www.sisu.chat/api/runtime/v1").unwrap();
    assert_eq!(result.base_url, "https://www.sisu.chat/api/runtime/v1");
    assert!(result.api_key.is_none());
    assert!(result.env_key.is_none());
    assert_ne!(result.api_base_url.as_deref(), Some("https://api.x.ai/v1"));
}
#[test]
