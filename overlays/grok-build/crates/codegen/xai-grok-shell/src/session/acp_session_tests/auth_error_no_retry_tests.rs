...
/// Live turns rebuild SamplerConfig via reconstruct_full_config; the SiSu
/// request-id injector must survive composition with TraceContextInjector.
#[tokio::test(flavor = "current_thread")]
#[serial_test::serial]
async fn reconstruct_full_config_access_point_stamps_request_id() {
    ...
    local
        .run_until(async {
            ...
            let injector = cfg
                .header_injector
                .as_ref()
                .expect("reconstruct_full_config must keep a header_injector");
            let mut headers = reqwest::header::HeaderMap::new();
            injector.inject(&mut headers);
            let first = headers
                .get("x-sisu-client-request-id")
                .expect("composed injector must add x-sisu-client-request-id")
                .clone();
            let mut headers2 = reqwest::header::HeaderMap::new();
            injector.inject(&mut headers2);
            let second = headers2
                .get("x-sisu-client-request-id")
                .expect("second inject must add x-sisu-client-request-id")
                .clone();
            assert_ne!(first, second, "fresh request-id per post()/inject()");
        })
        .await;
}
