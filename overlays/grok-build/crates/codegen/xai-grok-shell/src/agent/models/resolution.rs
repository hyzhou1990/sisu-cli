...
pub(crate) fn resolve_default_model(
    ...
    match &model_pref {
        None => {
            let (key, first) = first_or_fallback(&visible, catalog, cfg)?;
            Ok((key, first, config::ConfigSource::Default))
        }
        Some(pref) => {
            let found = visible
            ...
            if let Some((key, entry)) = found {
                Ok((key.clone(), entry.clone(), pref.source))
            } else {
                        ...
                        .or_else(|| visible.iter().find(|(_, m)| m.model == prev))
                {
                    ...
                    return Ok((key.clone(), entry.clone(), config::ConfigSource::Config));
                }
                let (key, first) = first_or_fallback(&visible, catalog, cfg)?;
                Ok((key, first, config::ConfigSource::Default))
            }
...
/// First selectable catalog row, or the bundled default. Access-point mode
/// never yields `crate::models::default_model()` (`grok-4.6`).
pub(crate) fn first_or_fallback(
    visible: &IndexMap<String, ModelEntry>,
    catalog: &IndexMap<String, ModelEntry>,
    cfg: &config::Config,
) -> Result<(String, ModelEntry), String> {
    if let Some((key, first)) = visible.first() {
        return Ok((key.clone(), first.clone()));
    }
    ...
    if crate::sisu_access_point::active() {
        return Err("no SiSu model available".to_string());
    }
...
mod access_point_tests {
    ...
    fn access_point_first_or_fallback_does_not_yield_grok_46() {
        ...
        let _flag = EnvGuard::set("SISU_ACCESS_POINT", "1");
        assert!(crate::sisu_access_point::active());
        let cfg = config::Config::default();
        let empty: IndexMap<String, ModelEntry> = IndexMap::new();
        let err = first_or_fallback(&empty, &empty, &cfg).expect_err("empty catalog must error");
        assert!(
            !err.to_string().contains("grok-4.6"),
            "access-point error must not mention grok-4.6: {err}"
        );
    }
}
