...
impl ModelsManager {
    ...
    fn reselect_current_model_if_missing(&self, config: &config::Config) {
        ...
        let (key, _, source) = {
            let cat = self.inner.catalog.read();
            let models = &cat.models;
            match resolve_default_model(config, models, self.is_session_auth()) {
                Ok(resolved) => resolved,
                Err(err) => {
                    tracing::warn!(error = %err, "cannot reselect default model");
                    return;
                }
            }
        };
    ...
    fn reselect_default_model(&self, config: &config::Config) {
        let (key, _, source) = {
            ...
            match resolve_default_model(config, models, self.is_session_auth()) {
                Ok(resolved) => resolved,
                Err(err) => {
                    tracing::warn!(error = %err, "cannot re-resolve default model");
                    return;
                }
            }
        };
