...
/// Access-point B-lite reuses `XAI_API_KEY` as the SiSu JWT — that is not an xAI key.
...
pub fn build_auth_methods(inputs: AuthMethodsBuildInputs<'_>) -> BuiltAuthMethods {
    ...
    if crate::sisu_access_point::active() {
        return build_access_point(crate::sisu_access_point::sisu_token().is_some());
    }
...
fn build_access_point(has_sisu_token: bool) -> BuiltAuthMethods {
    if has_sisu_token {
        BuiltAuthMethods {
            methods: vec![cached_token_auth_method()],
...
mod tests {
    ...
    #[test]
    #[serial]
    fn access_point_does_not_advertise_xai_api_key() {
    ...
    #[test]
    #[serial]
    fn access_point_with_sisu_token_does_not_advertise_grok_com() {
        let _ap = EnvGuard::set("SISU_ACCESS_POINT", "1");
        ...
        assert_eq!(first_kind(&built.methods), Some(AuthMethodKind::CachedToken));
        assert!(
            built
                .methods
                .iter()
                .all(|m| !AuthMethodKind::from_id(m.id()).needs_interactive_login()),
            "B-full with SISU_TOKEN must not auto-open grok.com login"
        );
    }

    #[test]
    #[serial]
    fn access_point_without_sisu_token_has_no_grok_com_method() {
        let _ap = EnvGuard::set("SISU_ACCESS_POINT", "1");
        ...
        assert!(
            built
                .methods
                .iter()
                .all(|m| AuthMethodKind::from_id(m.id()) != AuthMethodKind::GrokCom),
            "access-point must never advertise grok.com"
        );
    }
