...
pub(crate) const NPM_PACKAGE: &str = "@xai-official/grok";
...
// ─── SiSu version manifest (access-point mode) ─────────────────────────────
//
// SiSu access-point builds redirect the updater's *version source* from the
// upstream x.ai/GCS channel pointers to the SiSu runtime endpoint
// `GET {base}/cli/version`, which returns a small JSON document:
//
//     {"latest": "0.3.37", "stable": "0.3.37", "channel": "stable", ...}
//
// `latest` is the newest published `@stevezhou/sisu` npm version; `stable`
// mirrors it (single sisu channel) and feeds the same version cache slot the
// upstream stable pointer feeds. The binary download base (`cli_base_urls`)
// is deliberately NOT redirected: in access-point mode installs go through
// npm (`@stevezhou/sisu`), never through the upstream raw-binary CDN.
//
// All of this is inert unless `SISU_ACCESS_POINT=1`.

/// Default SiSu runtime API base; the version manifest lives at
/// `{base}/cli/version` on the wulabllm backend.
pub(crate) const SISU_DEFAULT_CLI_VERSION_BASE_URL: &str = "https://www.sisu.chat/api/runtime/v1";

/// Host pin for the version manifest base. Same validation family as the
/// other access-point runtime URLs: must be a SiSu runtime URL.
pub(crate) const SISU_CLI_VERSION_BASE_URL_ENV: &str = "SISU_CLI_VERSION_BASE_URL";

/// Resolve the SiSu version-manifest base. `None` when not in access-point
/// mode, or when the host pin is set but fails `is_sisu_runtime_url`
/// (refuse to leak update checks to an untrusted endpoint).
pub(crate) fn sisu_version_base_url() -> Option<String> {
    ...
    match std::env::var(SISU_CLI_VERSION_BASE_URL_ENV) {
        Ok(base) => {
                ...
                && xai_grok_shell::sisu_access_point::is_sisu_runtime_url(base)
            {
                ...
                tracing::warn!(
                    "{} ignored: not a SiSu runtime URL (/api/runtime/v1)",
                    SISU_CLI_VERSION_BASE_URL_ENV
                );
                None
            }
        }
        Err(_) => Some(SISU_DEFAULT_CLI_VERSION_BASE_URL.to_string()),
    }
}

#[derive(Debug, serde::Deserialize)]
pub(crate) struct SisuCliVersionManifest {
    latest: String,
    #[serde(default)]
    stable: Option<String>,
}

/// Fetch the SiSu version manifest. Returns Err when not in access-point
/// mode, the endpoint is unreachable, the body is not the expected JSON, or
/// `latest` fails the same strict semver validation the upstream channel
/// pointer gets.
pub(crate) async fn fetch_sisu_version_manifest() -> Result<SisuCliVersionManifest> {
    let Some(base) = sisu_version_base_url() else {
        anyhow::bail!("sisu version manifest unavailable: not a SiSu access-point runtime");
    };
    let url = format!("{}/cli/version", base);
    let client = xai_grok_extra_ca::build_reqwest_client(|builder| {
        builder.timeout(Duration::from_secs(15))
    })?;
    let mut req = client.get(&url);
    if let Some(Some(auth)) =
        xai_grok_shell::sisu_access_point::access_point_authorization_for_url(&url)
    {
    ...
    Ok(SisuCliVersionManifest { latest, stable })
}

pub(crate) async fn fetch_sisu_latest_version() -> Result<String> {
    Ok(fetch_sisu_version_manifest().await?.latest)
}

/// SiSu counterpart of the stable-pointer probe: best-effort, never required
/// for correctness (only feeds the `[stable]`/`[alpha]` channel label).
pub(crate) async fn try_fetch_sisu_stable_pointer() -> Option<String> {
    match fetch_sisu_version_manifest().await {
        Ok(manifest) => manifest.stable,
        Err(e) => {
            tracing::debug!("sisu stable pointer unavailable: {:#}", e);
            None
        }
    }
}
...
pub async fn fetch_latest_version(installer: &str, config: &UpdateConfig) -> Result<String> {
    // Access-point builds resolve every installer against the SiSu manifest:
    // npm/internal/gh-release distinctions are upstream-installer concepts.
    if xai_grok_shell::sisu_access_point::active() {
        return fetch_sisu_latest_version().await;
    }
    match installer {
...
pub(crate) async fn try_fetch_stable_pointer() -> Option<String> {
    if xai_grok_shell::sisu_access_point::active() {
        return try_fetch_sisu_stable_pointer().await;
    }
...
mod tests {
    ...
    /// Save/clear a fixed set of env vars for the test duration and restore
    /// them on drop. Env mutation is process-global, so every test touching
    /// it must be #[serial].
    struct EnvGuard {
    ...
    impl EnvGuard {
        fn isolate(vars: &'static [&'static str]) -> Self {
            let prev = vars.iter().map(|k| (*k, std::env::var_os(k))).collect();
            for k in vars {
    ...
    const SISU_ENV_VARS: &[&str] = &[
        "SISU_ACCESS_POINT",
        "SISU_CLI_VERSION_BASE_URL",
        "SISU_TOKEN",
        "XAI_API_KEY",
    ];
    ...
    #[test]
    #[serial]
    fn sisu_version_base_url_inert_without_access_point() {
        let g = EnvGuard::isolate(SISU_ENV_VARS);
        g.set(
            "SISU_CLI_VERSION_BASE_URL",
            "https://www.sisu.chat/api/runtime/v1",
        );
        assert!(!xai_grok_shell::sisu_access_point::active());
        assert_eq!(sisu_version_base_url(), None);
    }

    #[test]
    #[serial]
    fn sisu_version_base_url_defaults_to_sisu_chat() {
        let g = EnvGuard::isolate(SISU_ENV_VARS);
        g.set("SISU_ACCESS_POINT", "1");
        assert_eq!(
            sisu_version_base_url().as_deref(),
            Some(SISU_DEFAULT_CLI_VERSION_BASE_URL)
        );
    }

    #[test]
    #[serial]
    fn sisu_version_base_url_honors_host_pin() {
        let g = EnvGuard::isolate(SISU_ENV_VARS);
        g.set("SISU_ACCESS_POINT", "1");
        g.set(
            "SISU_CLI_VERSION_BASE_URL",
            "https://staging.sisu.chat/api/runtime/v1/",
        );
        assert_eq!(
            sisu_version_base_url().as_deref(),
            Some("https://staging.sisu.chat/api/runtime/v1")
        );
    }

    #[test]
    #[serial]
    fn sisu_version_base_url_rejects_untrusted_host_pin() {
        let g = EnvGuard::isolate(SISU_ENV_VARS);
        g.set("SISU_ACCESS_POINT", "1");
        for bad in [
            "https://api.x.ai/api/runtime/v1",
            "https://cli-chat-proxy.grok.com/api/runtime/v1",
            "https://evil.example/cli",
            "https://evil.example/api/runtime/v1?redirect=api.x.ai",
            "",
            "not a url",
        ] {
            g.set("SISU_CLI_VERSION_BASE_URL", bad);
            assert_eq!(
                sisu_version_base_url(),
                None,
                "host pin {bad:?} must be refused"
            );
        }
    }
    ...
    #[tokio::test]
    #[serial]
    async fn fetch_sisu_version_manifest_reads_wire_json() {
        use wiremock::matchers::{method, path};
        use wiremock::{Mock, MockServer, ResponseTemplate};

        let server = MockServer::start().await;
        Mock::given(method("GET"))
            .and(path("/api/runtime/v1/cli/version"))
            .respond_with(ResponseTemplate::new(200).set_body_json(serde_json::json!({
                "latest": "0.3.37",
                "stable": "0.3.37",
                "channel": "stable",
                "package": "@stevezhou/sisu",
            })))
            .mount(&server)
            .await;
        ...
        g.set(
            "SISU_CLI_VERSION_BASE_URL",
            &format!("{}/api/runtime/v1", server.uri()),
        );

        let manifest = fetch_sisu_version_manifest().await.expect("manifest");
        assert_eq!(manifest.latest, "0.3.37");
        assert_eq!(manifest.stable.as_deref(), Some("0.3.37"));
    }

    #[tokio::test]
    #[serial]
    async fn fetch_sisu_version_manifest_rejects_non_semver_latest() {
        use wiremock::matchers::{method, path};
        use wiremock::{Mock, MockServer, ResponseTemplate};
        ...
        assert!(fetch_sisu_version_manifest().await.is_err());
    }
}
