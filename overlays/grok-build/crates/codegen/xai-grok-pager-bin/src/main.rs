    ...
    if let Some(command) = args.command.take() {
        match command {
            ...
            Command::Login {
                ...
                if sisu_access_point::active() {
                    eprintln!("use `sisu login` — this binary does not run grok.com OAuth");
                    std::process::exit(2);
                }
                init_tracing_simple("cli");
                let _otel_guard = xai_grok_telemetry::otel_layer::otel_guard();
            ...
            Command::Logout => {
                if sisu_access_point::active() {
                    init_tracing_simple("cli");
                    if let Err(e) = xai_grok_shell::sisu_access_point::unlink_auth() {
                        eprintln!("Failed to clear SiSu auth: {e}");
                        std::process::exit(1);
                    }
                    eprintln!("Logged out");
                    xai_grok_shell::instrumentation::finalize_and_exit(0);
