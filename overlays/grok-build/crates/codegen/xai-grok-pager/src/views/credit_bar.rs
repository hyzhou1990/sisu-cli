...
/// One-line welcome quota summary (welcome screen tip slot), or `None` when
/// there is nothing meaningful to show.
///
/// Composed from the same pieces as [`format_usage_summary`]: usage
/// percentage, next reset, and the subscription tier. SiSu semantics: an
/// `is_unlimited` account may omit `creditUsagePercent` from the billing
/// response, which maps to `usage_pct == 0.0` — indistinguishable from a
/// fresh period. A zero percentage carries no information, so it is omitted
/// rather than rendered as a fake "0%"; the period end and tier still show.
pub fn welcome_quota_summary(
    balance: &CreditBalance,
    subscription_tier: Option<&str>,
) -> Option<String> {
    let mut parts: Vec<String> = Vec::new();
    if balance.usage_pct > 0.0 {
        parts.push(format!(
            "{}: {}%",
            balance.usage_label(),
            balance.usage_pct.floor() as i64
    ...
    if let Some(reset) = &balance.period_end_display {
        parts.push(format!("Next reset: {reset}"));
    }
    if let Some(tier) = subscription_tier.map(str::trim).filter(|t| !t.is_empty()) {
...
mod tests {
    ...
    #[test]
    fn welcome_quota_summary_composes_pct_reset_and_tier() {
        let b = CreditBalance {
            period_end_display: Some("June 8, 12:00".into()),
            period_type: Some("USAGE_PERIOD_TYPE_MONTHLY".into()),
            ..bal(42.5)
        };
        assert_eq!(
            welcome_quota_summary(&b, Some("Pro")).as_deref(),
            Some("Monthly limit: 42% · Next reset: June 8, 12:00 · Pro")
        );
    }

    #[test]
    fn welcome_quota_summary_omits_zero_pct_but_keeps_period_and_tier() {
        // SiSu is_unlimited: backend omits creditUsagePercent → usage_pct 0.0.
        // Must not render a fake "0%"; period end + tier still do.
        let b = CreditBalance {
            period_end_display: Some("June 8, 12:00".into()),
            ..bal(0.0)
        };
        assert_eq!(
            welcome_quota_summary(&b, Some("Unlimited")).as_deref(),
            Some("Next reset: June 8, 12:00 · Unlimited")
        );
    }

    #[test]
    fn welcome_quota_summary_none_when_nothing_to_show() {
        // No pct, no period, no tier → no line.
        let b = bal(0.0);
        assert_eq!(welcome_quota_summary(&b, None), None);
        // Empty/blank tier is dropped too.
        assert_eq!(welcome_quota_summary(&b, Some("  ")), None);
    }
}
