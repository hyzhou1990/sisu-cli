...
pub(super) enum VersionBadgeMode<'a> {
    /// Full badge: team | tier | api_key | **SiSu** VERSION+channel (right-aligned).
    Full { subscription_tier: Option<&'a str> },
    /// Hero footer: team | api_key | channel (right-aligned, gray).
    HeroFooter,
    /// Hero inline: **SiSu**  VERSION (left-aligned).
    HeroInline,
}
...
pub(super) fn render_version_badge(
    ...
    if show_api_key {
        if xai_grok_shell::sisu_access_point::active() {
            if let Some(email) = xai_grok_shell::sisu_access_point::account_email() {
                let label = match xai_grok_shell::sisu_access_point::account_plan() {
                    Some(plan) => format!("{email} \u{00b7} {plan}"),
                    None => email,
                };
                spans.push(Span::styled(label, Style::default().fg(theme.gray)));
                spans.push(sep.clone());
            }
        } else if is_api_key_auth {
            spans.push(Span::styled(
...
pub fn render_welcome(
    ...
    let mut result = match params.auth_state {
        AuthState::Pending { error } => {
            let label = if xai_grok_shell::sisu_access_point::active() {
                "SiSu"
            } else {
                params.login_label.unwrap_or("grok.com")
            };
        ...
        AuthState::Done if params.is_zdr_blocked => {
            ...
            let (menu_rects, post_flush_escapes) = render_welcome_blocked(
                ...
                buf,
                Some((
                    "SiSu is not yet available for this account.",
                    theme.gray_bright,
                )),
                &menu,
                params.selected,
...
fn render_welcome_trust(
    ...
    let lines = vec![
        ...
        Line::from(Span::styled(
            "SiSu may run or modify contents in this directory,",
            Style::default().fg(theme.gray),
        ))
        .alignment(Alignment::Center),
        Line::from(Span::styled(
...
mod tests {
    ...
    fn badge_text(mode: VersionBadgeMode<'_>, team: Option<&str>) -> String {
        badge_text_auth(mode, team, false)
    }

    fn badge_text_auth(mode: VersionBadgeMode<'_>, team: Option<&str>, is_api_key_auth: bool) -> String {
        let area = Rect::new(0, 0, 80, 1);
        let mut buf = Buffer::empty(area);
        render_version_badge(area, &mut buf, &Theme::current(), team, 0, is_api_key_auth, mode);
        (0..area.width)
    ...
    fn version_badge_carries_no_release_label() {
        ...
        assert!(full.contains("SiSu"), "full badge: {full:?}");
        assert!(inline.contains("SiSu"), "inline badge: {inline:?}");
        assert!(!full.contains("Grok Build"), "full badge must not name Grok: {full:?}");
        assert!(!inline.contains("Grok Build"), "inline badge must not name Grok: {inline:?}");
    ...
    #[test]
    #[serial_test::serial]
    fn version_badge_shows_sisu_account_email_when_access_point() {
        let _ap = xai_grok_test_support::EnvGuard::set("SISU_ACCESS_POINT", "1");
        let _email =
            xai_grok_test_support::EnvGuard::set("SISU_ACCOUNT_EMAIL", "ada@sisu.chat");
        let _plan = xai_grok_test_support::EnvGuard::set("SISU_ACCOUNT_PLAN", "pro");
        assert!(xai_grok_shell::sisu_access_point::active());

        let full = badge_text_auth(
            VersionBadgeMode::Full {
                subscription_tier: None,
            },
            None,
            true, // would be API-key chrome without access-point
        );
        assert!(
            full.contains("ada@sisu.chat"),
            "access-point badge must show host email: {full:?}"
        );
        assert!(
            full.contains("pro"),
            "access-point badge must show plan: {full:?}"
        );
        assert!(
    ...
    fn hero_box_height_accounts_for_borders_and_padding() {
        // Full Möbius is 9 lines. With menu_height=3:
        // right_col = 1 + 1 + 0 + 1 + 3 = 6, inner = max(9, 6) = 9.
        // hero_box_height = 2 (borders) + 2 (v_pad) + 9 = 13.
        ...
        assert!(layout.has_hero_box());
        assert_eq!(layout.hero_box.height, 13);
    }
    ...
    fn hero_box_announcement_clamped_when_tight() {
        ...
        // 9-line Möbius + menu 3 needs min_content_height 19.
        let area = Rect::new(0, 0, 100, 19);
        let a = long_ann();
