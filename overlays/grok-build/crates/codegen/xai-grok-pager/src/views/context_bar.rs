...
///
/// Prefer the current model's advertised window over a sticky session snapshot.
/// Switching SiSu-Pro (1M) → Grok (500k) must update the chrome immediately.
pub fn effective_context_total(
    model_window: Option<u64>,
    session_total: Option<u64>,
) -> Option<u64> {
    model_window
        .filter(|&t| t > 0)
        .or_else(|| session_total.filter(|&t| t > 0))
}
...
mod tests {
    ...
    #[test]
    fn effective_total_follows_model_window_not_sticky_session() {
        assert_eq!(
            effective_context_total(Some(500_000), Some(1_000_000)),
            Some(500_000)
        );
        assert_eq!(
            effective_context_total(Some(256_000), Some(1_000_000)),
            Some(256_000)
        );
        assert_eq!(effective_context_total(None, Some(1_000_000)), Some(1_000_000));
        assert_eq!(effective_context_total(Some(0), Some(1_000_000)), Some(1_000_000));
        assert_eq!(effective_context_total(Some(500_000), None), Some(500_000));
        assert_eq!(effective_context_total(None, None), None);
    }
