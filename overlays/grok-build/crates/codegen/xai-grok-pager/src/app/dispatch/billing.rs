...
const SISU_TOPUP_URL: &str = "https://www.sisu.chat";
const GROK_UPSELL_URL_UPGRADE: &str = "https://grok.com/supergrok?referrer=grok-build";
const GROK_UPSELL_URL_PAYG: &str = "https://grok.com?_s=usage";

/// URL for upgrading the subscription tier (SiSu top-up when access-point).
pub(crate) fn upsell_url_upgrade() -> &'static str {
    if xai_grok_shell::sisu_access_point::active() {
        SISU_TOPUP_URL
    } else {
        GROK_UPSELL_URL_UPGRADE
    }
}

/// URL for managing pay-as-you-go / on-demand spending / purchasing credits.
pub(crate) fn upsell_url_payg() -> &'static str {
    if xai_grok_shell::sisu_access_point::active() {
        SISU_TOPUP_URL
    } else {
        GROK_UPSELL_URL_PAYG
    }
}
...
fn open_supergrok_upsell(
    ...
    let sisu = xai_grok_shell::sisu_access_point::active();
    let (heading, source, modal_id_prefix) = match reason {
        UpsellReason::FreeUsageLimit if sisu => (
            "SiSu 配额已用尽。",
        ...
        UpsellReason::RestrictedCommand if sisu => (
            "需要 SiSu 配额。",
    ...
    let options = if sisu {
        vec![QuestionOption {
            label: "SiSu 充值".into(),
            description: format!("打开 {topup} 充值配额"),
            preview: None,
            id: Some(topup.into()),
        }]
    } else {
...
pub(super) fn dispatch_open_supergrok_url(app: &mut AppView) -> Vec<Effect> {
    ...
    if xai_grok_shell::sisu_access_point::active() {
        super::ctx::open_url_or_show(app, SISU_TOPUP_URL);
        return vec![];
    }
    let url = app
        .gate
        ...
        .unwrap_or(GROK_UPSELL_URL_UPGRADE);
