    ...
    if let Some(agent) = app.agents.get_mut(&agent_id) {
        ...
        if let Some((kind, body)) = notification {
            ...
            if kind != NotificationEventKind::TurnComplete || queue_empty {
                ...
                // Out-of-terminal counterpart to the OSC/bell rail above: a
                // terminal notification never reaches a user who switched
                // windows (or whose terminal lacks notification support
                // mid–long-turn). Focus/duration gating and debounce live in
                // `sisu_notify`; `kind == TurnComplete` here implies
                // `queue_empty` (see the enclosing condition), so queued
                // continuations stay silent.
                if kind == NotificationEventKind::TurnComplete {
                    crate::sisu_notify::maybe_notify_turn_complete(
                        app.notification_service.focus_tracker.is_focused(),
                        elapsed,
                        &agent.session.cwd,
                        crate::views::session_title::last_agent_message_line(agent)
                            .as_deref(),
