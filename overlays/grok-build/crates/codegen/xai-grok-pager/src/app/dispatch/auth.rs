...
/// Exit code the host `runTui` loops on to run `webLoginCommand` and respawn.
pub(crate) const SISU_LOGIN_EXIT_CODE: i32 = 10;
...
pub(super) fn dispatch_login(app: &mut AppView) -> Vec<Effect> {
    // Host is the only authenticator under access-point: quit so `sisu` can
    // run `webLoginCommand` and respawn. Never start grok OAuth / device-code.
    if xai_grok_shell::sisu_access_point::active() {
        app.quit_for_sisu_login = true;
        if let Some(tx) = &app.voice_cmd_tx {
            let _ = tx.try_send(xai_grok_voice::VoiceCommand::Shutdown);
        }
        let mut effects = super::task_result::unregister_all_active_sessions(app);
