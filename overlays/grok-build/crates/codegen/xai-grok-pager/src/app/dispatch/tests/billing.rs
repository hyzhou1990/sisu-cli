...
/// Access-point: every billing CTA URL is SiSu; copy never says SuperGrok.
#[serial_test::serial]
#[test]
fn access_point_billing_strings_use_sisu_not_supergrok() {
    ...
    assert!(
        !upgrade.to_ascii_lowercase().contains("supergrok"),
        "upgrade URL must not mention SuperGrok: {upgrade}"
    );
    assert!(payg.contains("sisu.chat"), "payg URL must be SiSu: {payg}");
    assert!(
        !payg.to_ascii_lowercase().contains("supergrok"),
        "payg URL must not mention SuperGrok: {payg}"
    );
    ...
    for opt in &q.options {
        let id = opt.id.as_deref().unwrap_or("");
        assert!(
            id.contains("sisu.chat"),
            "credit-limit option URL must be SiSu: {id}"
        );
        assert!(
            !id.to_ascii_lowercase().contains("supergrok"),
            "credit-limit option URL must not mention SuperGrok: {id}"
        );
        let blob = format!("{} {}", opt.label, opt.description);
        assert!(
            !blob.to_ascii_lowercase().contains("supergrok"),
            "credit-limit copy must not say SuperGrok: {blob}"
        );
    }
    ...
    let q = &agent_qv(&app).questions[0];
    let heading = &q.question;
    assert!(
        !heading.to_ascii_lowercase().contains("supergrok"),
        "free-usage heading must not say SuperGrok: {heading}"
    );
    assert!(
        heading.contains("SiSu") || heading.contains("配额") || heading.contains("充值"),
        "free-usage heading must be SiSu quota copy: {heading}"
    );
    for opt in &q.options {
        let blob = format!(
            "{} {} {}",
            opt.label,
            opt.description,
            opt.id.as_deref().unwrap_or("")
        );
        assert!(
            !blob.to_ascii_lowercase().contains("supergrok"),
            "free-usage option must not say SuperGrok: {blob}"
        );
        assert!(
            blob.contains("sisu.chat") || blob.contains("充值") || blob.contains("配额"),
            "free-usage option must point at SiSu top-up: {blob}"
        );
    }
}
