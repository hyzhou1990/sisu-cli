...
fn access_point_login_quits_for_host_sisu_login() {
    ...
    assert!(
        app.quit_for_sisu_login,
        "access-point /login must hand off to host"
    );
    assert!(
        effects.iter().any(|e| matches!(e, Effect::Quit)),
        "access-point /login must quit the pager: {effects:?}"
    );
    assert!(
        !effects
            .iter()
            .any(|e| matches!(e, Effect::Authenticate { .. })),
        "access-point /login must not start grok OAuth: {effects:?}"
    );
}
