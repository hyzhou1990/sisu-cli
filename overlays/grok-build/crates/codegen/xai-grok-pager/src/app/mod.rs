...
pub async fn run(
    ...
    let forced_exit_code = match &result {
        ...
        Ok(run_result) if run_result.quit_for_sisu_login => {
            Some(crate::app::dispatch::SISU_LOGIN_EXIT_CODE)
        }
        Ok(_) => Some(0),
        Err(_) => Some(1),
    };
    ...
    match result {
        Ok(run_result) => {
            ...
            if run_result.quit_for_sisu_login {
                // Host `runTui` respawns after `webLoginCommand` on this code.
                std::process::exit(crate::app::dispatch::SISU_LOGIN_EXIT_CODE);
            }
...
mod tests {
    ...
    #[test]
    fn cli_command_name_is_sisu() {
        use clap::CommandFactory;
        assert_eq!(PagerArgs::command().get_name(), "sisu");
    }
    #[test]
    fn cli_help_output_header() {
        ...
        assert_eq!(
            ...
            vec![
                "SiSu TUI — 思溯 · 思有所溯",
                "",
                "Usage: sisu [OPTIONS] [PROMPT] [COMMAND]",
                "",
                "Arguments:",
            ]
        );
