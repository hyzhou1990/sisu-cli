...
pub struct AppView {
    ...
    /// Access-point `/login`: exit so the host runs `sisu login` and respawns.
    pub quit_for_sisu_login: bool,
...
impl AppView {
    ...
    pub fn apply_auth_meta(&mut self, meta: &xai_grok_shell::auth::AuthMeta) {
        ...
        self.is_api_key_auth = if xai_grok_shell::sisu_access_point::active() {
            false
        } else {
            meta.auth_mode.as_deref().is_some_and(is_api_key_label)
                || meta
                    .subscription_tier
                    .as_deref()
