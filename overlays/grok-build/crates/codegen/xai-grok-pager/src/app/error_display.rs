...
fn classify(status: Option<u16>, wire: WireErrorType) -> Classified {
    if let Some(code) = status {
        if let Some(copy) = xai_grok_shell::sisu_access_point::billed_turn_copy(code) {
            return Classified {
                headline: format!("{} ({code})", copy.headline),
                action: Some(copy.action),
                default_why: Some(copy.why),
            };
        }
...
mod tests {
    ...
    #[test]
    #[serial_test::serial]
    fn access_point_401_is_sisu_login_not_request_denied() {
        let _ap = xai_grok_test_support::EnvGuard::set("SISU_ACCESS_POINT", "1");
        let formatted = format_request_failure(Some(401), Some("api"), "Not authenticated");
        assert_eq!(formatted.headline, "Sign in required (401)");
        assert!(formatted.message().contains("sisu login"));
        assert!(!formatted.message().contains("Request denied"));
    }

    #[test]
    #[serial_test::serial]
    fn access_point_402_is_sisu_quota_not_grok_com() {
        let _ap = xai_grok_test_support::EnvGuard::set("SISU_ACCESS_POINT", "1");
        let formatted = format_request_failure(Some(402), Some("api"), "quota_exhausted");
        assert_eq!(formatted.headline, "Quota exhausted (402)");
        assert!(formatted.message().contains("https://www.sisu.chat"));
        assert!(!formatted.message().to_ascii_lowercase().contains("grok.com"));
        assert!(!formatted.message().to_ascii_lowercase().contains("supergrok"));
    }
}
